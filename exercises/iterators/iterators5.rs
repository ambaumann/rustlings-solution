...
fn count_iterator(map: &HashMap<String, Progress>, value: Progress) -> usize {
    ...
    map.iter().fold(0, |acc, (key, progrees)| if progrees == &value {acc + 1} else {acc})
}
...
fn count_collection_iterator(collection: &[HashMap<String, Progress>], value: Progress) -> usize {
    ...
    //     { "variables2": Complete, ... }, ... ]
    collection.iter().fold(0, |totalacc, map| {
        totalacc + map.iter().fold(0, |acc, (key, progrees)| if progrees == &value {acc + 1} else {acc})
        //Same as: totalacc + count_iterator(map, value)
    })
}
